...
def main():
    ...
    tmpdir = "%s/bitcoinmoney_test_runner_%s" % (args.tmpdirprefix, datetime.datetime.now().strftime("%Y%m%d_%H%M%S"))
    os.makedirs(tmpdir)
    ...
    if not (enable_wallet and enable_utils and enable_bitcoind):
        print("No functional tests to run. Wallet, utils, and bitcoinmoneyd must all be enabled")
...
def run_tests(test_list, src_dir, build_dir, exeext, tmpdir, jobs=1, enable_coverage=False, args=[], combined_logs_len=0):
    ...
    try:
        if subprocess.check_output(["pidof", "bitcoinmoneyd"]) is not None:
            print("%sWARNING!%s There is already a bitcoinmoneyd process running on this system. Tests may fail unexpectedly due to resource contention!" % (BOLD[1], BOLD[0]))
    ...
    if "BITCOINMONEYD" not in os.environ:
        os.environ["BITCOINMONEYD"] = build_dir + '/src/bitcoinmoneyd' + exeext
        os.environ["BITCOINMONEYCLI"] = build_dir + '/src/bitcoinmoney-cli' + exeext
...
class RPCCoverage():
    ...
    commands per `bitcoinmoney-cli help` (`rpc_interface.txt`).
